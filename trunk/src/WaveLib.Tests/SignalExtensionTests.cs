...
namespace WaveletStudio.WaveLib.Tests
{
    ...
    public class SignalExtensionTests
    {
        ...
        { 
            var points = new ILArray<double>(new double[] { 2, 6, 1, 6, 5, 0, 4, 5, 3, 4 });
            ...
            points = new ILArray<double>(new double[] { 1, 2 });
        ...
        public void TestExtendSymmetricHalfPoint()
        {
            ...
            var expected = new ILArray<double>(new double[] { 1, 6, 2, 2, 6, 1, 6, 5, 0, 4, 5, 3, 4, 4, 3, 5 });
            Assert.IsTrue(extended.SequenceEqual(expected));

            points = new ILArray<double>(new double[] { 2, 6, 1 });
            extended = SignalExtension.Extend(points, SignalExtension.ExtensionMode.SymmetricHalfPoint, 1);
            expected = new ILArray<double>(new double[] { 2, 2, 6, 1, 1 });
            Assert.IsTrue(extended.SequenceEqual(expected));

            points = new ILArray<double>(new double[] { 2, 6, 1, 6, 5, 0});
            extended = SignalExtension.Extend(points, SignalExtension.ExtensionMode.SymmetricHalfPoint, 1);
            expected = new ILArray<double>(new double[] { 2, 2, 6, 1, 6, 5, 0, 0 });
            Assert.IsTrue(extended.SequenceEqual(expected));
        }
        ...
        public void TestExtendSymmetricWholePoint()
        {
            ...
            var expected = new ILArray<double>(new double[] { 6, 1, 6, 2, 6, 1, 6, 5, 0, 4, 5, 3, 4, 3, 5, 4 });
            Assert.IsTrue(extended.SequenceEqual(expected));

            points = new ILArray<double>(new double[] { 2, 6, 1 });
            extended = SignalExtension.Extend(points, SignalExtension.ExtensionMode.SymmetricWholePoint, 1);
            expected = new ILArray<double>(new double[] { 6, 2, 6, 1, 6 });
            Assert.IsTrue(extended.SequenceEqual(expected));

            points = new ILArray<double>(new double[] { 2, 6, 1, 6, 5, 0 });
            extended = SignalExtension.Extend(points, SignalExtension.ExtensionMode.SymmetricWholePoint, 1);
            expected = new ILArray<double>(new double[] { 6, 2, 6, 1, 6, 5, 0, 5 });
            Assert.IsTrue(extended.SequenceEqual(expected));
        }
        ...
        {
            var points = new ILArray<double>(new double[] {2, 6, 1, 6, 5, 4, 5, 3, 4});
            var extended = SignalExtension.Extend(points, SignalExtension.ExtensionMode.AntisymmetricWholePoint, 3);
            var expected = new ILArray<double> (new double[] {-2, 3, -2, 2, 6, 1, 6, 5, 4, 5, 3, 4, 5, 3, 4});
            Assert.IsTrue(extended.SequenceEqual(expected));

            points = new ILArray<double>(new double[] { 2, 6, 1 });
            extended = SignalExtension.Extend(points, SignalExtension.ExtensionMode.AntisymmetricWholePoint, 1);
            expected = new ILArray<double>(new double[] { -2, 2, 6, 1, -4 });
            Assert.IsTrue(extended.SequenceEqual(expected));

            points = new ILArray<double>(new double[] { 2 });
            extended = SignalExtension.Extend(points, SignalExtension.ExtensionMode.AntisymmetricWholePoint, 1);
            expected = new ILArray<double>(new double[] { -2, 2, -2 });
            Assert.IsTrue(extended.SequenceEqual(expected));

            points = new ILArray<double>(new double[] { });
            extended = SignalExtension.Extend(points, SignalExtension.ExtensionMode.AntisymmetricWholePoint, 1);
            expected = new ILArray<double>(new double[] { 0, 0 });
            Assert.IsTrue(extended.SequenceEqual(expected));
        }
        ...
        public void TestExtendPeriodicPadding()
        {
            ...
            var expected = new ILArray<double>(new double[] { 5, 3, 4, 2, 6, 1, 6, 5, 0, 4, 5, 3, 4, 2, 6, 1 });
            Assert.IsTrue(extended.SequenceEqual(expected));

            points = new ILArray<double>(new double[] { 2, 6, 1 });
            extended = SignalExtension.Extend(points, SignalExtension.ExtensionMode.PeriodicPadding, 1);
            expected = new ILArray<double>(new double[] { 1, 2, 6, 1, 2 });
            Assert.IsTrue(extended.SequenceEqual(expected));
        }
        ...
        public void TestExtendZeroPadding()
        {
            ...
            var expected = new ILArray<double>(new double[] { 0, 0, 0, 2, 6, 1, 6, 5, 0, 4, 5, 3, 4, 0, 0, 0 });
            Assert.IsTrue(extended.SequenceEqual(expected));

            points = new ILArray<double>(new double[] { 2, 6, 1 });
            extended = SignalExtension.Extend(points, SignalExtension.ExtensionMode.ZeroPadding, 1);
            expected = new ILArray<double>(new double[] { 0, 2, 6, 1, 0 });
            Assert.IsTrue(extended.SequenceEqual(expected));
        }
        ...
        public void TestExtendSmoothPadding0()
        {
            ...
            var expected = new ILArray<double>(new double[] { 2, 2, 2, 2, 6, 1, 6, 5, 0, 4, 5, 3, 4, 4, 4, 4 });
            Assert.IsTrue(extended.SequenceEqual(expected));
        ...
        public void TestExtendSmoothPadding1()
        {
            ...
            var expected = new ILArray<double>(new double[] { -10, -6, -2, 2, 6, 1, 6, 5, 0, 4, 5, 3, 4, 5, 6, 7 });
            Assert.IsTrue(extended.SequenceEqual(expected));

            points = new ILArray<double>(new double[] { 7, 6, 1, 6, 5, 0, 4, 5, 3, 1 });
            extended = SignalExtension.Extend(points, SignalExtension.ExtensionMode.SmoothPadding1, 3);
            expected = new ILArray<double>(new double[] { 10, 9, 8, 7, 6, 1, 6, 5, 0, 4, 5, 3, 1, -1, -3, -5 });
            Assert.IsTrue(extended.SequenceEqual(expected));
