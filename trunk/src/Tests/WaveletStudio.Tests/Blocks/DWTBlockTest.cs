...
namespace WaveletStudio.Tests.Blocks
{
    ...
    public class DWTBlockTest
    {
        ...
        public void TestDwt()
        {
            var samples = new double[] { 5, 6, 7, 8, 1, 2, 3, 4 };
            var signal = new Signal(samples);
            var wavelet = MotherWavelet.LoadFromName("daub4");
            var output = DWT.ExecuteDWT(signal, wavelet, 2, SignalExtension.ExtensionMode.SymmetricHalfPoint);
            foreach (var level in output)
            {
                Console.WriteLine("Approximation Level " + level.Index + ":  " + level.Approximation.Aggregate("", (c, i) => c + " " + i));
                Console.WriteLine("Details Level " + level.Index + ":  " + level.Details.Aggregate("", (c, i) => c + " " + i));
            }
        }
