...
namespace WaveletStudio.Tests.Blocks
{
    ...
    public class ShiftBlockTest
    {
        ...
        [TestMethod]
        public void TestShiftBlockExecute2()
        {
            var block = new ShiftBlock { Delay = 1.7 };
            var signalBlock = new ImportFromTextBlock
                                  {
                                      Text = "2.1, 3.2, -1, -1.3, -100, -2", 
                                      SignalStart = 2
                                  };
            signalBlock.ConnectTo(block);
            signalBlock.Execute();

            Console.WriteLine(block.OutputNodes[0].Object[0].Start);
            Assert.AreEqual(3.7, block.OutputNodes[0].Object[0].Start);
        }
    }
}
